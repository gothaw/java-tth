...
public class Main {
  ...
  public static Map<String, Long> getWordCountFromGameTitlesDeclaratively(List<Score> scores) {
    return scores.stream()
      // Map the stream to the game title
      // Map that to it's lowercase version, so things are case insensitive
      .map(score -> score.getGame().toLowerCase())
    // Map that to a new String array of each word in the title (See imperative implementation)
      .map(title -> title.split("\\W+"))
    // Flatten those words into the stream
      .flatMap(title -> Stream.of(title))
    // Collect a grouping of the word to count
